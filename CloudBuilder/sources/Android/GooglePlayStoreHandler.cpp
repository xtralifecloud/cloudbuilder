...
bool CGooglePlayStoreHandler::GetJavaGooglePlayHandler(JNIEnv* &env, jclass &clazz, jobject &instance, CInternalResultHandler *onError) {
	...
	clazz = env->FindClass("com/clanofthecloud/cloudbuilder/GooglePlayStoreHandler");
	if (!clazz) { InvokeHandler(onError, enInternalError, "JNI: GooglePlayStoreHandler class not found"); return false; }
	jmethodID method = env->GetStaticMethodID(clazz, "getInstance", "()Lcom/clanofthecloud/cloudbuilder/GooglePlayStoreHandler;");
 ...
 */
extern "C" JNIEXPORT jint JNICALL Java_com_clanofthecloud_cloudbuilder_GooglePlayStoreHandler_postPurchase(JNIEnv* aEnv, jclass aClass, jint javaHandlerId, jstring jsonStr) {
