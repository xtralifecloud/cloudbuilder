...
namespace CloudBuilder {
	 ...
	 */
	struct CGooglePlayStoreHandler: CStoreGlue {
		 ...
		 */
		virtual void GetInformationAboutProducts(const CotCHelpers::CHJSON *productIds, CInternalResultHandler *onFinished);
		virtual void LaunchPurchase(const CotCHelpers::CHJSON *productInfo, CInternalResultHandler *onFinished);
	...
	private:
		...
		void CallStandardJavaMethod(JNIEnv *env, jclass clazz, jobject instance, const char *methodName,
			const CotCHelpers::CHJSON *params, CInternalResultHandler *handler);
	};
}
